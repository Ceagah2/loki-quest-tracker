...
export function LokiSection({ tracker }: { tracker: TrackerAPI }) {
  const { lokiTierDone, lokiSel, lokiUnlocked, selectLoki, completeLoki, clearLokiSel, toggleCard, isOpen } =
    tracker;
        ...
        return (
          <div
            ...
            className={`rounded-lg border overflow-hidden transition-colors ${
                ...
                ? "border-green-800/40 bg-cave-card"
                : unlocked
                ? "border-rune bg-cave-card hover:border-rune-border"
          ...
          >
            ...
            >
              ...
              <div className="flex items-center gap-2">
                ...
                {unlocked && !done && (
                  <span className="text-[10px] px-2 py-0.5 rounded-full bg-teal-900/20 text-teal-400 border border-teal-800/30">
                    Em andamento
                  </span>
                )}
                <span
                  className={`text-stone-600 text-sm transition-transform duration-200 ${open ? "rotate-180" : ""}`}
                >
                  ▾
                </span>
              </div>
            ...
            {open && unlocked && (
              <div className="px-3.5 pb-3 border-t border-rune">
                <p className="text-[11px] text-stone-500 mt-2.5 mb-2">
                  Selecione a quest que o jogo sorteou:
                </p>
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center gap-3 px-3 py-2 rounded-md border text-left transition-all ${
                          isDoneQ
                            ? "bg-green-900/15 border-green-800/35 cursor-not-allowed"
                            : isSel
                            ? "bg-teal-900/15 border-teal-700/40"
                            : done
                            ? "border-rune bg-cave-card2 opacity-40 cursor-not-allowed"
